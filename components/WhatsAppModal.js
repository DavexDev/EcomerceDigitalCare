...
export default function WhatsAppModal() {
  ...
  useEffect(() => {
    ...
    document.addEventListener('open-whatsapp', handler);
    return () => document.removeEventListener('open-whatsapp', handler);
  }, []);
  ...
  const handleEnviar = async () => {
    ...
    // Guardar lead en Supabase (sin bloquear el envío a WhatsApp)
    try {
    ...
    const texto = encodeURIComponent(
      `Hola, estoy interesado en: ${producto}\n${mensaje}`
    );
    window.open(`https://wa.me/502${sucursal}?text=${texto}`, '_blank');
    setOpen(false);
    setEnviando(false);
  };
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setOpen(false)}>
      <div className="modal">
        ...
        <div className="modal-body">
          <p>Producto seleccionado: <strong>{producto}</strong></p>
